...
describe('Challenge 4 - Redux Store', () => {
  ...
  describe('Redux store action creator and reducer', () => {
    it('has an action creator that returns an object', () => {
    ...
    it('the action creator returns an object with `type` and `dogs` fields', () => {
      const returnValue = gotDogs(doggies);
    ...
    it('the `dogs` field of the action should match the dogs passed in to the action creator', () => {
      const returnValue = gotDogs(doggies);
    ...
    it('the reducer returns a new state with dogs after dispatching `addDogs`', () => {
      store.dispatch(gotDogs(doggies));
      const newState = store.getState();

      // your expect assertion here...
      expect(newState.dogs).to.have.length(2);
    });
  });
});
